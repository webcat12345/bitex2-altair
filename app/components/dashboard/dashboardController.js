angular
    ...
    .controller('dashboardCtrl', [
        ...
        'variables',
        'public_apis',
        function ($rootScope,$scope,$interval,$timeout,sale_chart_data,user_data,variables,public_apis,$window,$q) {

            var self = this;
        // trades history
            $scope.trades_history = {
                count : 0 , 
                price : [],
                bc_amount : [],
                qc_amount : []
            }
            $scope.trades_history.price.push('Price');
            $scope.trades_history.bc_amount.push('BC_Amount');
            $scope.trades_history.qc_amount.push('QC_Amount');

            public_apis.GetTradesHistory(100 , function(res) {
                if (res) {
                    $scope.trades_history.count = res.count;
                    for (var i = 0; i < res.results.length; i++) {                        
                        $scope.trades_history.price.push(res.results[i].price);
                        $scope.trades_history.bc_amount.push(res.results[i].bc_amount);
                        $scope.trades_history.qc_amount.push(res.results[i].qc_amount);                        
                    }                 
                    drawC3Chart('c3_chart_spline' , $scope.trades_history.price);
                    drawC3Chart('c3_chart_bc_amount' , $scope.trades_history.bc_amount);
                    drawC3Chart('c3_chart_qc_amount' , $scope.trades_history.qc_amount);

                }
            });
            function drawC3Chart (element_id , array) {
                    console.log('chart draw');
                    var c3chart_spline_id = element_id;
        ...
        // bids and asks
            ...
            public_apis.GetBidsList(100 , function(res) {
                if (res) {
                    $scope.bids_and_asks.bids.count = res.count;
                    for (var i = 0; i < res.results.length; i++) {
                        $scope.bids_and_asks.bids.list.push(res.results[i]);
                    }                 
                }
            });

            public_apis.GetAsksList(100 , function(res) {
                if (res) {
                    $scope.bids_and_asks.asks.count = res.count;
                    for (var i = 0; i < res.results.length; i++) {
                        $scope.bids_and_asks.asks.list.push(res.results[i]);
                    }
                }
            });
        ...
        // large chart
            ...
            // // $scope.sale_chart_data = $scope.bids_and_asks.bids.list;
            // // console.log($scope.sale_chart_data);
